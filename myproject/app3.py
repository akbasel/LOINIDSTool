...
def upload():
    ...
    if file:
        try:
            ...
            uploaded_loin_xml = file.read().decode('utf-8')
...
def conversion_page():
    ...
    if uploaded_loin_xml is not None:
        try:
            # Load desite content from file or any other source
            desite_file_path = "C:/Users/MSI-NB/Desktop/LOIN/desiterule.qa.xml"
            with open(desite_file_path, "r", encoding="utf-8") as desite_file:
                desite_content = desite_file.read()

            # Perform the conversion here
            formatted_xml, new_script_code, desite_root = convert_loin_to_ids(uploaded_loin_xml, desite_content)

            # Instead of rendering the template, directly return the formatted XML
            response = make_response(formatted_xml)
            response.headers["Content-Disposition"] = "attachment; filename=formatted_xml.xml"
        ...
        return "No uploaded LOIN XML. Please upload a file first."
